# Authorization Component chart

The Helm chart installs Authorization Component and by default the following dependencies using subcharts:
...
```bash
helm repo add Authorization Component https://raw.githubusercontent.com/ConductionNL/Authorization-Component/master/api/helm/
helm repo update
```
...
```bash
helm install my-Authorization Component ConductionNL/Authorization-Component --version 0.1.0
```
...
| `settings.subdomain` | the subdomain of the installation excluding www. | `Authorization Component` |
| `settings.subpath` | Any sub path to follow the domain, like /api/v1 | `Authorization Component` |
