...
class CommongroundApplicationAuthenticator extends AbstractGuardAuthenticator
{
    ...
    public function getUser($credentials, UserProviderInterface $userProvider)
    {
        ...
        if ($apiToken != $this->params->get('app_application_key') && $this->params->get('auth_provider_application') != 'ac') {
            return;
        }
        elseif($this->params->get('auth_provider_application') == 'ac' && !$this->validateJWT($apiToken)){
            return;
        }
    ...
    public function validateJWT(string $bearer): bool
    {
        ...
        foreach($applications as $application){
            if($application instanceof Application && in_array($clientId, $application->getClientIds())){
                break;
            }
        }
        ...
        $maxAge = new \DateTime("now - 1 hours");
